...
let words = []; // To store the words fetched from the JSON file
let lastCategory = null; // To track the last selected category

// Function to fetch words from a JSON file (assuming words are stored in a JSON format)
async function loadWords() {
    const response = await fetch('/data/word_sets.json'); // Replace with the correct path
    const data = await response.json();

    // Assuming data has two sections: Chemistry and Physics
    const categories = ['Chemistry', 'Physics'];
    words = getRandomWordsFromCategories(data, categories);

    initializeGrid(); // Start the game after loading words
}

// Function to get 10 random words from the available categories
function getRandomWordsFromCategories(data, categories) {
    let selectedWords = [];
    while (selectedWords.length < 10) {
        const category = categories[Math.floor(Math.random() * categories.length)];
        if (category !== lastCategory) {
            // Randomly pick a word from the category
            const word = data[category][Math.floor(Math.random() * data[category].length)];
...
loadWords(); // Fetch words and start the game
